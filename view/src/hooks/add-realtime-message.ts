...
export function useAddRealtimeMessage() {
  ...
  useEffect(() => {
    ...
    socket.on("newMessage", (data) => {
      ...
      // ✅ unread count increase only if msg not from active chat
      const activeChatId = useStore.getState().activeChatId;
      if (activeChatId !== parsedResponse.data.chatId) {
        incrementUnreadCount(parsedResponse.data.chatId);
      }
    });
  ...
  }, [socket, pushMessage, moveChatToTop, incrementUnreadCount]);
}

...
export function ChatItems({ chats, loading, search }: ChatItemsProps) {
  ...
  useEffect(() => {
    if (!hasResolvedInitialGroup && !groupLoading) {
      setHasResolvedInitialGroup(true);
    }
    if (groupLoading && hasResolvedInitialGroup) {
      setGroupLoading(false);
    }
  ...
  return (
    <ScrollArea
      className={cn(
        "h-[70vh] w-full max-w-full overflow-y-auto overflow-x-hidden px-5",
        chatType === "chats" ? "pb-4" : ""
      )}
    >
      ...
      {chatType !== "chats" && chatType !== "templates" ? (
        <div className="bg-sidebar sticky bottom-0 mt-auto flex flex-col items-center justify-between gap-5 border-t px-3 py-6 w-full max-w-full">
          <SelectChats
